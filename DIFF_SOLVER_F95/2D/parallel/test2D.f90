...
program test2D
  ...
  real (kind=8) :: eps
  integer :: kmax, its
  parameter (eps = 1.0d-12, kmax = 1000)
	...
	! now solve and produce output information 
	 if (flag == 0) then
  	...
  	elseif (flag == 1) then
	    ...
	    call BTCSmatrix(P,alpha,tmax,m,Nt,ibeg,iend)
	    ! Something is wrong. Preconditioning makes the its grow...
	 	call cpu_time(t1)
  		do j=2,Nt
...
! something is wrong with this part of the code. Output solution is not correct
